<?php 
    ...
    session_start(); 
       ...
       $email = $_GET['email'];
        ...
        if(!$row || count($row) == 0)
        {
        // This intentionally does not support business account creation
	        header('Location: http://menyouapp.com/appCreateAccount.php?email='.$_GET['email'].'&passhash='.$_GET['passhash']);
        } else {
        if($row) 
        { 
